...
public class DynmapApi {
    ...
    private final ApiCache<Map<String, Location>> cache =
            new ApiCache<>(5000);
    ...
    public CompletableFuture<Map<String, Location>> getPlayers() {
        System.out.println("[Dynmap] Fetching players.json...");
        Map<String, Location> cached = cache.get("players");

        if (cached != null) {
            EarthlingClient.LOGGER.debug(
                    "[Dynmap] Using cached player list ({} players).",
                    cached.size()
            );
        ...
        return http.getRaw(URL).thenApply(body -> {
            ...
            EarthlingClient.LOGGER.info(
                    "[Dynmap] Loaded {} visible players from Dynmap.",
                    players.size()
            );
